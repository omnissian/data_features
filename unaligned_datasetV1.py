...
#Image.open(path+original_names[i]+'_real_A'+extension)
        ...
        if (index>=len(self.list_names)):
            ...
            A=torch.Tensor((np.array(Image.open(A_path),dtype=np.uint8)))# .convert('RGB')) # correct  <<<----------
            B=torch.Tensor((np.array(Image.open(B_path).convert('RGB'),dtype=np.uint8)))# .convert('RGB')) # RGB by default??? correct  <<<----------
            A=A.permute(2,0,1)
            B=B.permute(2,0,1)
