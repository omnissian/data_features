...
# NumberCrop_numtype_Input_maskname
# NumberCrop - number of crop
# numtype - number of type source (google, ersi map, etc)
# maskname - name of the mask that it connects to
    ...
    for name in (names_folders):
        with Image.open(path_inputs+name[:-4]+"/"+"google_sat.png")  as big_google_sat, \
            ...
            for n in range(num_crops):
                ...
                #-------------------
                for i in range(len(list_original_inputs)):
                    input_img=Image.fromarray(list_original_inputs[i][width_new:width_new+width_resolution,height_new:height_new+height_resolution,:])
                    input_img.save(path_save_inputs+str(n)+"_"+name[:-4]+"input"+str(i)+".png")
                mask_new=Image.fromarray(mask[width_new:width_new+width_resolution,height_new:height_new+height_resolution,:])
                mask_new.save(path_save_masks+str(n)+"_"+name[:-4]+"mask"+".png")
                # print("debug")
                #
...
print("check saved files in folders")
